    ...
    struct SliderHandle: View {
        ...
        var body: some View {
            Circle()
                ...
                .animation(.defaultSpring, value: isDragging)
        }
    }
    ...
    var body: some View {
        GeometryReader { reader in
            ZStack(alignment: .leading) {
                SliderPath()
                    .stroke(
                        Color(uiColor: .quaternaryLabel).opacity(0.5),
                ...
                TickMarks(tickCount: config.tickCount)
                    .stroke(
                        Color(uiColor: .quaternaryLabel).opacity(0.5),
                        style: StrokeStyle(lineWidth: config.tickMarkWidth, lineCap: .round, lineJoin: .round)
                    )
                ...
                ZStack {
                    SliderHandle(
                        hour: minHour,
                        offset: translateHourToWidth(hour: minHour, reader: reader),
                        diameter: config.handleDiameter
                    ) { value in
                        let newValue = min(
                            translateWidthToHour(width: value.location.x, reader: reader),
                            maxHour - config.minimumDistance
                        )
                        if minHour != newValue {
                            minHour = newValue
                    ...
                    SliderHandle(
                        hour: maxHour,
                        offset: translateHourToWidth(hour: maxHour, reader: reader),
                        diameter: config.handleDiameter
                    ) { value in
                        let newValue = max(
                            translateWidthToHour(width: value.location.x, reader: reader),
                            minHour + config.minimumDistance
                        )
                        if maxHour != newValue {
                            maxHour = newValue
                            feedbackGenerator.impactOccurred()
