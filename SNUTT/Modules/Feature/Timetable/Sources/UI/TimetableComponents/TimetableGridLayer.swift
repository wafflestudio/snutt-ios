...
struct TimetableGridLayer: View {
    ...
    func verticalPaths(in containerSize: CGSize) -> Path {
        let weekCount = viewModel.weekCount
        let weekWidth = viewModel.getWeekWidth(in: containerSize, weekCount: weekCount)
        return Path { path in
    ...
    func horizontalHourlyPaths(in containerSize: CGSize) -> Path {
        let hourCount = viewModel.hourCount
        let hourHeight = viewModel.getHourHeight(in: containerSize, hourCount: hourCount)
        return Path { path in
    ...
    var weeksHStack: some View {
        HStack(spacing: 0) {
            let visibleWeeksSorted = viewModel.visibleWeeks
    ...
    var hoursVStack: some View {
        let minHour = viewModel.startingHour
        let maxHour = viewModel.endingHour
...
#Preview {
    TimetableGridLayer(viewModel: .init())
}
