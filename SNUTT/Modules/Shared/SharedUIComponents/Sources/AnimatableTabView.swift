...
public class AnimatableUITabBarController<T: TabItem>: UITabBarController, UITabBarControllerDelegate {
    ...
    private func selectTab(_ tab: T) {
        ...
        selectedTabItem = tab
        selectedIndex = tabItems.firstIndex(of: selectedTabItem) ?? 0
    ...
    init(
        ...
        delegate = self
        viewControllers = tabScenes.map { UIHostingController(rootView: $0.rootView) }
    }
    ...
    struct PreviewWrapper: View {
        ...
        var body: some View {
            AnimatableTabView(selectedTab: $selectedTab) {
                ...
                TabScene(tabItem: PreviewTabItem.search, rootView: ColorView(color: .blue))
